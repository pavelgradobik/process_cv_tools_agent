...
class ReActResumeAgent:
    def __init__(
        ...
        self.session_stats = {
            "total_queries": 0,
            "tool_usage": {},
            "session_start": datetime.now(),  # Initialize session_start
        }
    ...
    def _create_tools(self) -> List[FunctionTool]:
        ...
        try:
            if self.store.index:
                ...
                self.logger.info("✅ Resume tools loaded successfully")
            else:
                self.logger.warning("⚠️ No indexed data - resume tools unavailable")

        except Exception as e:
            self.logger.error(f"❌ Error loading resume tools: {e}")
        ...
        try:
            ...
            self.logger.info("✅ General knowledge tools loaded successfully")

        except ImportError:
            self.logger.warning("⚠️ DuckDuckGo not available - web search tools unavailable")
        except Exception as e:
            self.logger.error(f"❌ Error loading general tools: {e}")
        ...
        try:
            ...
            self.logger.info("✅ Calculator tools loaded successfully")

        except Exception as e:
            self.logger.error(f"❌ Error loading calculator tools: {e}")
...
Remember: You're helping HR professionals and recruiters find the best candidates efficiently."""
    ...
    async def chat(self, message: str, context: Optional[Context] = None) -> Dict[str, Any]:
        try:
            ...
            conversation_entry = {
                ...
                "tools_used": [trace["tool"] for trace in reasoning_trace]
            }
            ...
            self.logger.info(f"Response completed in {response_time:.2f}s")
    ...
    def get_session_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_queries": self.session_stats.get("total_queries", 0),
            "tools_available": len(self.tools),
            "tool_usage": self.session_stats.get("tool_usage", {}),
...
def quick_agent_setup(store: LlamaIndexStore) -> ReActResumeAgent:
    try:
        return create_react_agent(store, verbose=True, timeout=60.0)
    except Exception as e:
        logger.error(f"Failed to create quick agent setup: {e}")
        return ReActResumeAgent(
            store=store,
            verbose=False,
            timeout=30.0,
...
def production_agent_setup(store: LlamaIndexStore) -> ReActResumeAgent:
